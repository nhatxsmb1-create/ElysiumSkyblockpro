...
text = re.sub(
    r'            List<Double> hist = new ArrayList<>\(info\.getPriceHistory\(\)\);\s*hist\.add\(currentPrice\); // Add current to end\s*StringBuilder sparkline = new StringBuilder\(\);\s*String\[\] blocks = \{.*?\};\s*for \(double p : hist\) \{.*?\s*\}\s*lore\.add\(sparkline\.toString\(\)\);',
    new_chart,
    text,
    flags=re.DOTALL
)
